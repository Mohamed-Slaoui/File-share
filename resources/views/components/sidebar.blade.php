    ...
    <div class="h-full px-3 pb-4 overflow-y-auto bg-white dark:bg-gray-800">
        <ul class="space-y-2 font-medium">
            ...
            @auth
                ...
                <li>
                    <a href="{{ route('myFiles', Auth::user()->role_id == 1 ? "": Auth::user()->id ) }}"
                        class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
            ...
            <li>
                <button type="button"
                    ...
                    <img src="{{ asset('icons/category-alt.svg') }}" class="w-5 opacity-45 group-hover:opacity-100"
                        alt="">
                ...
                <ul id="dropdown-example" class="block py-2 space-y-2">
                    @foreach ($cats as $c)
                        <li>
                            <a href="{{ route('filterByCategory',$c->id) }}"
                                class="flex items-center w-full p-2 text-gray-600 transition duration-75 rounded-lg pl-11 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700">{{ $c->name }}</a>
                        </li>
                    @endforeach
